    ...
    <section id="testimonials" className="section-container">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {testimonialsData.map((item, index) => (
          ...
          >
            <div className="border border-white/5 bg-slate-900/40 group relative flex h-full flex-col justify-between overflow-hidden rounded-xl p-6 transition-all duration-300 hover:-translate-y-1 hover:border-emerald-300/10 hover:bg-slate-900/60 sm:p-8 shadow-md">
              <Quote className="absolute right-6 top-6 h-8 w-8 text-emerald-300/5 transition-colors duration-300 group-hover:text-emerald-300/15" />
              ...
              <div className="relative z-10 mt-6">
                <div className="flex items-center gap-3">
                  <img
                    ...
                    alt={item.name}
                    loading="lazy"
                    className="h-10 w-10 rounded-full border border-emerald-300/15 object-cover bg-slate-900/60"
                  />
                  <div>
                    <h4 className="text-sm font-semibold text-slate-100">{item.name}</h4>
                    <p className="text-xs text-slate-400">
                      {item.role}, <span className="text-emerald-300/90">{item.company}</span>
                    </p>
                  </div>
                </div>
